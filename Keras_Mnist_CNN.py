...
def data_preprocess(datapath):
    ...
    for root,dirs,files in os.walk(datapath):
        for f in files:
            ...
            img=(np.array(img)/255).reshape(1,28,28) # 作正規化與reshape
...
model.add(Conv2D(filters=32,kernel_size=(3,3),padding='same',input_shape=(28,28,1),activation='relu')) # 建立卷基層
...
model.add(Dense(units=10,activation='softmax')) # 使用 softmax,將結果分類 units=10,10類
model.summary()
